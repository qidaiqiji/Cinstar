<!DOCTYPE html>
<html lang="en">
<head>
    <?php include('../inc/meta.php'); ?>
    <?php include('../inc/rem.php'); ?>
    <link rel="stylesheet" href="../css/detail.css">
    <title>Outdoor display board | outdoor LED sign boards, outdoor digital screen</title>
    <meta name="description" content="Cinstar FT series outdoor display board supports front or rear service access for specific installation. The high brightness and great stability of the outdoor digital screen enable it a preferred outdoor LED sign boards for your commercial outdoor advertising business." />
    <meta name="keywords" content="outdoor display board, outdoor LED sign boards, outdoor digital screen, commercial LED sign board, front service LED display, LED advertising display board, LED outdoor advertising screens" />
</head>
<body class="page-index .menu-open">
    <!-- 头部开始 -->
    <?php include('../inc/top.php'); ?>
    <!-- 头部结束 -->
    <div class="container">
        <div class="detail-title ft-bg bg">
            <div class="title">
                <span>FT SERIES</span>
            </div>
            <div class="top-title">DOOH Advertising LED Video Display Specialist</div>
        </div>
        <h1>Cinstar is a professional experienced outdoor display board, outdoor LED sign boards, outdoor digital screen, commercial LED sign board, front service LED display, LED advertising display board, LED outdoor advertising screens, LED screen, LED display, LED video screen, LED video wall, LED video display, LED screen display, LED display screen, LED screen wall, LED display wall, LED wall display, LED wall video, rental LED display, commercial LED display, creative LED display manufacturer.</h1>
        <!-- 产品参数 -->
        <div class="specification">
            <div class="spec-detail">
                <table>
                    <tbody>
                        <tr class="thead">
                            <td colspan="2">Specifications</td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Pitch (mm):
                            </td>
                            <td>
                                4.81 / 5.95 / 6.67 / 8 / 10
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Brightness (nits):
                            </td>
                            <td>
                                up to 7500
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Refresh Rate(Hz):    
                            </td>
                            <td>
                                >1920
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Application: 
                            </td>
                            <td>
                                Outdoor Fixed
                            </td>
                        </tr>
                    </tbody>
                </table>
                <div class="key-feature">
                    <div>Key Features</div>
                    <ul>
                        <li>Specially designed for branding & digital advertising</li>
                        <li>Built to withstand severe weather conditions</li>
                        <li>Excellent brightness & contrast levels</li>
                        <li>Wide viewing angles</li>
                        <li>Easy front or rear service access for options</li>
                        <li>Advanced monitoring & reporting system – Optional</li>    
                    </ul>
                </div>
            </div>
            <div class="product-img small-big">
                <img src="../images/products/Commercial-LED/FT SERIES.jpg" alt="FT SERIES"/>
            </div>
        </div>
        <!-- 产品详情 -->
        <div class="product-detail ft-series">
            <div class="detail">
                <div class="img-part half">
                    <img src="../images/products/Commercial-LED/FT Series1.jpg" alt="FT SERIES"/>
                </div>
                <div class="desc-part half">
                    <h2 class="title">GREAT DURABILITY AND STABILITY</h2>
                    <p class="mobile-hide">
                    Cinstar FT series LED video display was developed specifically for commercial billboards and outdoor digital advertising markets.
                    </p>
                    <p>
                    Designed for fixed outdoor installations, FT series LED video panels are robust and durable, high qualified to withstand any harsh environment factors such as constant vibrations, direct UV rays, varying temperatures and severe weather conditions. The FT series LED systems are suitable for 24/7 operation.
                    </p>
                </div>
            </div>
            <div class="detail">
                <div class="desc-part half">
                    <h2 class="title">FRONT AND REAR MAINTENANCE FOR OPTIONS</h2>
                    <p>
                    Cinstar FT series LED video display supports front or rear service access for specific installation. The fully front installation and maintenance panel design easier your installation procedures and much convenient to maintain. It is the perfect choice for against the wall installation or any other environment where requires no rear maintenance space. 
                    </p>
                </div>
                <div class="img-part half">
                    <img src="../images/products/Commercial-LED/FT Series2.jpg" alt="FT SERIES"/>
                </div>
            </div>
            <div class="detail lower">
                <div class="img-part half">
                    <img src="../images/products/Commercial-LED/FT Series3.jpg" alt="FT SERIES"/>
                </div>
                <div class="desc-part half">
                    <h2 class="title">HIGH BRIGHTNESS WITH WIDE VIEWING ANGLE</h2>
                    <p>
                    Cinstar FT series LED video display equipped with high brightness LEDs (DIP or SMD for options) to make sure the screen displaying vivid and clear image even in direct sunlight. The wide viewing angle catches more eyes on the screen and deliver brilliant visual enjoyment to the passing-bys, greatly maximize the commercial value.
                    </p>
                </div>
            </div>
            <div class="detail lower">
                <div class="desc-part half">
                    <h2 class="title">ADVANCED MONITORING AND REPORTING SYSTEM</h2>
                    <p>
                    The advanced reporting system allows clients to remotely monitor the performance of the entire LED display and individual panels live from any destination. It detects any inconsistencies and failures and feedback the results in real time when they occur.
                    </p>
                </div>
                <div class="img-part half">
                    <img src="../images/products/Commercial-LED/FT Series4.jpg" class="special" alt="FT SERIES"/>
                </div>
            </div>
        </div>
        <!-- SERIES FEATURES -->
        <div class="features">
            <div class="sub-title">
                <span>FT SERIES FEATURES</span>
            </div>
            <div class="iconsList">
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-thumbs-up"></i>
                    </div>
                    <p class="icon-desc">Designed for branding & digital advertising</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="iconfont">&#xe68d;</i>
                    </div>
                    <p class="icon-desc">Built to withstand severe weather conditions</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="iconfont">&#xe621;</i>
                    </div>
                    <p class="icon-desc">Excellent brightness & contrast levels</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-eye"></i>
                    </div>
                    <p class="icon-desc">Wide viewing angles</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="iconfont">&#xe600;</i>
                    </div>
                    <p class="icon-desc">Easy front or rear service access for options</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-wifi"></i>
                    </div>
                    <p class="icon-desc">Advanced monitoring & reporting system</p>
                </div>

            </div>
        </div>
        <!-- showCases -->
        <div class="show-cases">
            <div class="sub-title">
                <span>FT SERIES SHOWCASE</span>
            </div>
            <div class="img-wrap">
                <div class="caseItem">
                    <img src="../images/fixed/19-fixed-case.jpg" alt="China Chengdu Shopping Center FT10"/>
                    <a class="case-title">
                        <span class="current">China Chengdu Shopping Center FT10 LED Video Display</span>
                    </a>
                </div>
                <div class="caseItem">
                    <img src="../images/fixed/6-fixed-case.jpg" alt="Norway Mobile Trailer Screen FT6.67"/>
                    <a class="case-title">
                        <span class="current">Norway Mobile Trailer Screen FT6.67 LED Video Display</span>
                    </a>
                </div>
            </div>
        </div>
        <div class="table-detail">
            <div class="sub-title">
                <span>technical specification</span>
            </div>
            <div class="table-wrap">
                <table>
                    <tbody>
                        <tr class="thead">
                            <td></td>
                            <td>FT4.8</td>
                            <td>FT5.9</td>
                            <td>FT6.67</td>
                            <td>FT8</td>
                            <td>FT10</td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Pitch (mm)
                            </td>
                            <td>
                                4.81
                            </td>
                            <td>
                                5.95
                            </td>
                            <td>
                                6.67
                            </td>
                            <td>
                                8
                            </td>
                            <td>
                                10
                            </td>
                        </tr>
                        <tr>
                            <td>
                                LED Type
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Size (WxH)/(mm)
                            </td>
                            <td>
                                750x750 or Customized
                            </td>
                            <td>
                                750x750 or Customized
                            </td>
                            <td>
                                960x960 or Customized
                            </td>
                            <td>
                                960x960 or Customized
                            </td>
                            <td>
                                960x960 or Customized
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Per Panel
                            </td>
                            <td>
                                156x156
                            </td>
                            <td>
                                126x126
                            </td>
                            <td>
                                144x144
                            </td>
                            <td>
                                120x120
                            </td>
                            <td>
                                96x96
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Material
                            </td>
                            <td>
                                Steel or Aluminum
                            </td>
                            <td>
                                Steel or Aluminum
                            </td>
                            <td>
                                Steel or Aluminum
                            </td>
                            <td>
                                Steel or Aluminum
                            </td>
                            <td>
                                Steel or Aluminum
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Weight (kg)
                            </td>
                            <td>
                                40
                            </td>
                            <td>
                                40
                            </td>
                            <td>
                                54
                            </td>
                            <td>
                                54
                            </td>
                            <td>
                                54
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Module Size (WxH)/(mm)
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                320x320
                            </td>
                            <td>
                                320x320
                            </td>
                            <td>
                                320x320
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Brightness (nit)
                            </td>
                            <td>
                                >5000
                            </td>
                            <td>
                                >5000
                            </td>
                            <td>
                                >5000
                            </td>
                            <td>
                                >5000
                            </td>
                            <td>
                                >5000
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Refresh Rate (Hz)
                            </td>
                            <td>
                                >1920
                            </td>
                            <td>
                                >1920
                            </td>
                            <td>
                                >1920
                            </td>
                            <td>
                                >1920
                            </td>
                            <td>
                                >1920
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Gray Scale (bit) 
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Viewing Angle (H/V) (°) 
                            </td>
                            <td>
                                140/120
                            </td>
                            <td>
                                140/120
                            </td>
                            <td>
                                140/120
                            </td>
                            <td>
                                140/120
                            </td>
                            <td>
                                140/120
                            </td>
                        </tr>
                        <tr>
                            <td>
                                AC Operating Voltage (V)
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Avg. Power Consumption (W)
                            </td>
                            <td>
                                240/sqm
                            </td>
                            <td>
                                243/sqm
                            </td>
                            <td>
                                243/sqm
                            </td>
                            <td>
                                256/sqm
                            </td>
                            <td>
                                226/sqm
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Max. Power Consumption (W)
                            </td>
                            <td>
                                810/sqm
                            </td>
                            <td>
                                826/sqm
                            </td>
                            <td>
                                729/sqm
                            </td>
                            <td>
                                768/sqm
                            </td>
                            <td>
                                679/sqm
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Storage Temperature (℃) 
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Operating Temperature (℃)
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                        </tr>
                        <tr>
                            <td>
                                IP Rating (Front/Rear)
                            </td>
                            <td>
                                IP65/IP54
                            </td>
                            <td>
                                IP65/IP54
                            </td>
                            <td>
                                IP65/IP54
                            </td>
                            <td>
                                IP65/IP54
                            </td>
                            <td>
                                IP65/IP54
                            </td>
                        </tr>
                        <tr>
                            <td>
                                LED Lifetime (H)
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Module Maintenance 
                            </td>
                            <td>
                                Front or Rear
                            </td>
                            <td>
                                Front or Rear
                            </td>
                            <td>
                                Front or Rear
                            </td>
                            <td>
                                Front or Rear
                            </td>
                            <td>
                                Front or Rear
                            </td>
                        </tr>
                        <tr>
                            <td>
                                PSU & Others Maintenance
                            </td>
                            <td>
                                Front or Rear
                            </td>
                            <td>
                                Front or Rear
                            </td>
                            <td>
                                Front or Rear
                            </td>
                            <td>
                                Front or Rear
                            </td>
                            <td>
                                Front or Rear
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Certificate
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
        <div class="related">
            <div class="head-title">
                <span>RELATED PRODUCTS</span>
            </div>
            <div class="product-container">
                <div class="product-item">
                    <a class="thumb" href="../product-detail/mc-series.php">
                        <img src="../images/home/MC SERIES.jpg" alt="mc-series"/>
                    </a>
                    <a class="list-title" href="../product-detail/mc-series.php">
                        <span class="current">MC SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/db-series.php">
                        <img src="../images/home/DB Series.jpg" class="shorter" alt="db-series"/>
                    </a>
                    <a class="list-title" href="../product-detail/db-series.php">
                        <span class="current">DB SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/hd-series.php">
                        <img src="../images/home/HD SERIES.jpg" class="short" alt="hd-series"/>
                    </a>
                    <a class="list-title" href="../product-detail/hd-series.php">
                        <span class="current">HD SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/tr-series.php">
                        <img src="../images/home/TR SERIES.jpg" class="short" alt="tr-series"/>
                    </a>
                    <a class="list-title" href="../product-detail/tr-series.php">
                        <span class="current">TR SERIES</span><span class="more">View More</span>
                    </a>
                </div>
            </div>
        </div>
        <?php include('../inc/foot.php'); ?>
            <!-- 底部结束 -->
    </div>
    <?php include('../inc/js.php'); ?>
    <script>GetCurStyle("products");</script>
</body> 
</html>