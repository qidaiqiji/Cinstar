<!DOCTYPE html>
<html lang="en">
<head>
    <?php include('../inc/meta.php'); ?>
    <?php include('../inc/rem.php'); ?>
    <link rel="stylesheet" href="../css/detail.css">
    <title>LED Rental | Rental LED Display, LED Screen Rental, Stage LED | Cinstar-led</title>
    <meta name="description" content="Cinstar CR series offers advanced, high-performance rental LED display for stage rental applications. The lightweight and slim panel is great for LED screen rental. If you are looking for LED rental for touring & event production, CR series is the right choice." />
    <meta name="keywords" content="LED rental, rental LED display, LED screen rental, stage LED, stage rental LED screen, rental event LED wall, LED video wall rental" />
</head>
<body class="page-index .menu-open">
    <!-- 头部开始 -->
    <?php include('../inc/top.php'); ?>
    <!-- 头部结束 -->
    <div class="container">
        <div class="detail-title cr-bg bg">
            <div class="title">
                <span>CR SERIES</span>
            </div>
            <div class="top-title">Creative Solution LED Video Display</div>
        </div>
        <h1>Cinstar is a professional experienced digital display screens, LED rental, rental LED display, LED screen rental, stage LED, stage rental LED screen, rental event LED wall, LED video wall rental, LED screen, LED display, LED video screen, LED video wall, LED video display, LED screen display, LED display screen, LED screen wall, LED display wall, LED wall display, LED wall video, rental LED display, commercial LED display, creative LED display manufacturer.</h1>
        <!-- 产品参数 -->
        <div class="specification">
            <div class="spec-detail">
                <table>
                    <tbody>
                        <tr class="thead">
                            <td colspan="2">Specifications</td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Pitch (mm):
                            </td>
                            <td>
                                2.6 / 2.9 / 3.9 / 4.8
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Brightness (nits):
                            </td>
                            <td>
                                up to 5500
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Refresh Rate(Hz):    
                            </td>
                            <td>
                                1920-3840
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Application: 
                            </td>
                            <td>
                                Fixed/Rental
                            </td>
                        </tr>
                    </tbody>
                </table>
                <div class="key-feature">
                    <div>Key Features</div>
                    <ul>
                        <li>Fast locking system for quick and easy assembly</li>
                        <li>Robust panel frame and durable giant top locks</li>
                        <li>Separate and exchangeable power and control unit</li>
                        <li>Indoor and outdoor options, front or rear maintenance</li>
                        <li>Designed to create convex and concave curved setup</li>
                        <li>Seamless image quality, superb visual performance</li>    
                    </ul>
                </div>
            </div>
            <div class="product-img">
                <img src="../images/products/Rental-LED/CR-SERIES.jpg" alt="CR-SERIES"/>
            </div>
        </div>
        <!-- 产品详情 -->
        <div class="product-desc">
            <div class="left-img img-shorter">
                <img src="../images/products/Rental-LED/CR-Case-Show.jpg" alt="CR-Case-Show"/>
            </div>
            <div class="right-desc text-long">
                <h2 class="title">Creative Rental Solutions</h2>
                <p>Cinstar CR series LED screen display offers optimal, great-performance rental LED panel board for both indoor and outdoor rental applications.</p>
                <p>Designed especially for AV rental and entertainment industry, CR series LED video panels are extremely lightweight and slim but tough and durable. The LED system equipped with user-friendly features like ergonomic handles for convenient handling and operation, fast build-up and easy alignment.</p>  
                <p>When it comes to massive LED video display, the giant robust top lock allows suspension more than 10m and enables one-man installation on site which greatly accelerates preparation work and lower operation cost.</p> 
                <p>Featuring integrated power unit and magnetic module front service simplify the maintenance job and servicing procedures for specific events.</p>
                <p>The versatile CR series LED video display is capable of hanging, ground stacking or curved installations. It is ideal for live production, corporate event, award ceremony and music festival etc. One for all solution meets all your expectations on high profile and maximizes your return on investment.</p>
            </div>
        </div>
        <!-- SERIES FEATURES -->
        <div class="features">
            <div class="sub-title">
                <span>CR Series Features</span>
            </div>
            <div class="iconsList">
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-clock-o"></i>
                    </div>
                    <p class="icon-desc">Fast locking system for rapid construction</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-lock"></i>
                    </div>
                    <p class="icon-desc">Robust panel frame and durable giant locks</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-random"></i>
                    </div>
                    <p class="icon-desc">Separate and exchangeable power and control unit</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="iconfont">&#xe68d;</i>
                    </div>
                    <p class="icon-desc">Indoor and outdoor models available</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-check"></i>
                    </div>
                    <p class="icon-desc">Hanging, ground stacking or curved installations</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-thumbs-up"></i>
                    </div>
                    <p class="icon-desc">Seamless image quality, superb visual performance</p>
                </div>

            </div>
        </div>
        <!-- showCases -->
        <div class="show-cases">
            <div class="sub-title">
                <span>CR Series Showcase</span>
            </div>
            <div class="img-wrap">
                <div class="caseItem">
                    <img src="../images/rental/5-rental-case.jpg" alt="Mercedes Brisbane CR3.9 Outdoor"/>
                    <a class="case-title">
                        <span class="current">Mercedes Brisbane CR3.9 Outdoor LED Video Display</span>
                    </a>
                </div>
                <div class="caseItem">
                    <img src="../images/rental/6-rental-case.jpg" alt="Norway Concert Touring CR3.9"/>
                    <a class="case-title">
                        <span class="current">Norway Concert Touring CR3.9 LED Video Display</span>
                    </a>
                </div>
            </div>
        </div>
        <div class="table-detail">
            <div class="sub-title">
                <span>Technical Specification</span>
            </div>
            <div class="table-wrap">
                <table>
                    <tbody>
                        <tr class="thead">
                            <td></td>
                            <td>CR2.6</td>
                            <td>CR2.9</td>
                            <td>CR3.9</td>
                            <td>CR3.9(Outdoor)</td>
                            <td>CR4.8(Outdoor)</td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Pitch (mm)
                            </td>
                            <td>
                                2.6
                            </td>
                            <td>
                                2.97
                            </td>
                            <td>
                                3.91
                            </td>
                            <td>
                                3.91
                            </td>
                            <td>
                                4.81
                            </td>
                        </tr>
                        <tr>
                            <td>
                                LED Type
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Size (WxH)/(mm)
                            </td>
                            <td>
                                500x500
                            </td>
                            <td>
                                500x500
                            </td>
                            <td>
                                500x500
                            </td>
                            <td>
                                500x500
                            </td>
                            <td>
                                500x500
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Per Panel
                            </td>
                            <td>
                                192x192
                            </td>
                            <td>
                                168x168
                            </td>
                            <td>
                                128x128
                            </td>
                            <td>
                                128x128
                            </td>
                            <td>
                                104x104
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Material
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Weight (kg)
                            </td>
                            <td>
                                7.6
                            </td>
                            <td>
                                7.6
                            </td>
                            <td>
                                7.6
                            </td>
                            <td>
                                8.7
                            </td>
                            <td>
                                8.7
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Module Size (WxH)/(mm)
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Brightness (nit)
                            </td>
                            <td>
                                1000
                            </td>
                            <td>
                                1200
                            </td>
                            <td>
                                1200
                            </td>
                            <td>
                                5000
                            </td>
                            <td>
                                5000
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Refresh Rate (Hz)
                            </td>
                            <td>
                                >1920
                            </td>
                            <td>
                                >1920
                            </td>
                            <td>
                                >1920
                            </td>
                            <td>
                                >1920
                            </td>
                            <td>
                                >1920
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Gray Scale (bit) 
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Viewing Angle (H/V) (°) 
                            </td>
                            <td>
                                160/140
                            </td>
                            <td>
                                160/140
                            </td>
                            <td>
                                160/140
                            </td>
                            <td>
                                160/140
                            </td>
                            <td>
                                160/140
                            </td>
                        </tr>
                        <tr>
                            <td>
                                AC Operating Voltage (V)
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Avg. Power Consumption (W)
                            </td>
                            <td>
                                240/sqm
                            </td>
                            <td>
                                210/sqm
                            </td>
                            <td>
                                218/sqm
                            </td>
                            <td>
                                236/sqm
                            </td>
                            <td>
                                243/sqm
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Max. Power Consumption (W)
                            </td>
                            <td>
                                720/sqm
                            </td>
                            <td>
                                630/sqm
                            </td>
                            <td>
                                654/sqm
                            </td>
                            <td>
                                708/sqm
                            </td>
                            <td>
                                729/sqm
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Storage Temperature (℃) 
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Operating Temperature (℃)
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                        </tr>
                        <tr>
                            <td>
                                IP Rating (Front/Rear)
                            </td>
                            <td>
                                IP40/IP21
                            </td>
                            <td>
                                IP40/IP21
                            </td>
                            <td>
                                IP40/IP21
                            </td>
                            <td>
                                IP65/IP54
                            </td>
                            <td>
                                IP65/IP54
                            </td>
                        </tr>
                        <tr>
                            <td>
                                LED Lifetime (H)
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Module Maintenance 
                            </td>
                            <td>
                                Front/Rear
                            </td>
                            <td>
                                Front/Rear
                            </td>
                            <td>
                                Front/Rear
                            </td>
                            <td>
                                Front/Rear
                            </td>
                            <td>
                                Front/Rear
                            </td>
                        </tr>
                        <tr>
                            <td>
                                PSU & Others Maintenance
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Certificate
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
        <div class="related">
            <div class="sub-title">
                <span>Related Products</span>
            </div>
            <div class="product-container">
                <div class="product-item">
                    <a class="thumb" href="../product-detail/lm-series.php">
                        <img src="../images/home/LM SERIES.jpg" alt="lm-series"/>
                    </a>
                    <a class="list-title" href="../product-detail/lm-series.php">
                        <span class="current">LM SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/vx-series.php">
                        <img src="../images/home/VX Series.jpg" class="shorter" alt="vx-series"/>
                    </a>
                    <a class="list-title" href="../product-detail/vx-series.php">
                        <span class="current">VX SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/tr-series.php">
                        <img src="../images/home/TR SERIES.jpg" class="short" alt="tr-series"/>
                    </a>
                    <a class="list-title" href="../product-detail/tr-series.php">
                        <span class="current">TR SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/ei-series.php">
                        <img src="../images/home/EI Series.jpg"  class="shorter" alt="ei-series"/>
                    </a>
                    <a class="list-title" href="../product-detail/ei-series.php">
                        <span class="current">EI SERIES</span><span class="more">View More</span>
                    </a>
                </div>
            </div>
        </div>
                <!-- 底部开始 -->
        <?php include('../inc/foot.php'); ?>
            <!-- 底部结束 -->
    </div>
    <?php include('../inc/js.php'); ?>
    <script>GetCurStyle("products");</script>
</body> 
</html>