<!DOCTYPE html>
<html lang="en">
<head>
    <?php include('../inc/meta.php'); ?>
    <?php include('../inc/rem.php'); ?>
    <link rel="stylesheet" href="../css/detail.css">
    <title>Outdoor LED Display Board | Cinstar-led, Digital Billboard Advertising</title>
    <meta name="description" content="Cinstar outdoor LED display board DB series adopts high strength die-cast aluminum and magnalium cabinet for better digital billboard advertising.  Its excellent heat dissipation makes the cooling system not needed even for giant display wall." />
    <meta name="keywords" content="outdoor LED display board, digital billboard advertising, large screen display, outdoor LED wall screens, electric LED signs, advertising display board, outdoor LED advertising screens" />
</head>
<body class="page-index .menu-open">
    <!-- 头部开始 -->
    <?php include('../inc/top.php'); ?>
    <!-- 头部结束 -->
    <div class="container">
        <div class="detail-title db-bg bg">
            <div class="title">
                <span>DB SERIES</span>
            </div>
            <div class="top-title">High Profile Media LED Video Display</div>
        </div>
        <h1>Cinstar is a professional experienced outdoor LED display board, digital billboard advertising, large screen display, outdoor LED wall screens, electric LED signs, advertising display board, outdoor LED advertising screens, LED screen, LED display, LED video screen, LED video wall, LED video display, LED screen display, LED display screen, LED screen wall, LED display wall, LED wall display, LED wall video, rental LED display, commercial LED display, creative LED display manufacturer.</h1>
        <!-- 产品参数 -->
        <div class="specification">
            <div class="spec-detail">
                <table>
                    <tbody>
                        <tr class="thead">
                            <td colspan="2">Specifications</td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Pitch (mm):
                            </td>
                            <td>
                                4 / 5 / 6 / 8 / 10
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Brightness (nits):
                            </td>
                            <td>
                                up to 7000
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Refresh Rate(Hz):    
                            </td>
                            <td>
                                >1920
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Application: 
                            </td>
                            <td>
                                Outdoor Fixed
                            </td>
                        </tr>
                    </tbody>
                </table>
                <div class="key-feature">
                    <div>Key Features</div>
                    <ul>
                        <li>Unparalleled IP68 protection of module</li>
                        <li>High brightness, high refresh rate and great grey scale</li>
                        <li>Ultra slim and light weight cabinet</li>
                        <li>All-weather around-the-clock operation </li>
                        <li>Advanced heat dissipation system</li>
                        <li>Front or rear services for options</li>
                    </ul>
                </div>
            </div>
            <div class="product-img">
                <img src="../images/products/Commercial-LED/DB-SERIES.jpg" alt="DB-SERIES"/>
            </div>
        </div>
        <!-- 产品详情 -->
        <div class="product-desc">
            <div class="left-img half-width"> 
                <img src="../images/products/Commercial-LED/DB-Case-Show.jpg" alt="DB-Case-Show"/>
            </div>
            <div class="right-desc half-width" style="background:#fff;color:#000">
                <h2 class="title">High Profile Media Solutions</h2>
                <p>Cinstar DB series LED video display is specially designed for high end outdoor commercial advertising applications. Pixel pitch available from 4mm to 10mm.
The display system is with high brightness (up to 7000nits), high refresh rate (>2880Hz) and high grey scale (16bit), which ensuring the superb visual performance and excellent contrast ratio in any light condition.</p>
                <p>Cinstar DB series LED video display adopts high strength die-cast aluminum and magnesium cabinet, only 32kg/sqm, easier to install. Its excellent heat dissipation makes the cooling system not needed even for a giant LED screen display.</p> 
            </div>
        </div>
        <div class="product-desc">
            <div class="right-desc half-width" style="background:#fff;color:#000">
                <h2 class="title">High Stability & Energy Saving</h2>
                <p>Featuring the patented LEDs, Cinstar DB series LED display module reaches the highest IP rating (IP68) and secures its working temperature from +100 to -40. The unparalleled stability enables it the best choice for harsh environment installations like seaside and tropical areas.</p>
                <p>Cinsatr DB series LED video display offers the energy saving solution for option. By adopting the customized LEDs, power supply and driver IC, the brightness can reach up to 6000-7000nits while its power consumption can be at least 50% less. The operation cost is tremendously lowered for the long run. </p> 
                <p>Cinstar DB series LED video display offers both front and rear service access for options.</p>
            </div>
            <div class="left-img half-width"> 
                <img src="../images/products/Commercial-LED/DB-Case-Show2.jpg" alt="DB-Case-Show"/>
            </div>
            
        </div>
        <!-- SERIES FEATURES -->
        <div class="features">
            <div class="sub-title">
                <span>DB Series Features</span>
            </div>
            <div class="iconsList">
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-tint"></i>
                    </div>
                    <p class="icon-desc">Unparalleled IP68 protection of module</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-eye"></i>
                    </div>
                    <p class="icon-desc">High brightness, high refresh rate and great grey scale</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-check"></i>
                    </div>
                    <p class="icon-desc">Ultra slim and light weight cabinet</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-clock-o"></i>
                    </div>
                    <p class="icon-desc">All-weather around-the-clock operation</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-random"></i>
                    </div>
                    <p class="icon-desc">Advanced heat dissipation system</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="iconfont">&#xe600;</i>
                    </div>
                    <p class="icon-desc">Front or rear services for options</p>
                </div>

            </div>
        </div>
        <!-- showCases -->
        <div class="show-cases">
            <div class="sub-title">
                <span>DB Series Showcase</span>
            </div>
            <div class="img-wrap">
                <div class="caseItem">
                    <img src="../images/fixed/1-fixed-case.jpg" alt="New Zealand Shopping Mall DB6"/>
                    <a class="case-title">
                        <span class="current">New Zealand Shopping Mall DB6 LED Screen Display</span>
                    </a>
                </div>
                <div class="caseItem">
                    <img src="../images/fixed/18-fixed-case.jpg" alt="Russia Mosco Airport DB10"/>
                    <a class="case-title">
                        <span class="current">Russia Mosco Airport DB10 LED Screen Display</span>
                    </a>
                </div>
            </div>
        </div>
        <div class="table-detail">
            <div class="sub-title">
                <span>Technical Specification</span>
            </div>
            <div class="table-wrap">
                <table>
                    <tbody>
                        <tr class="thead">
                            <td></td>
                            <td>DB4</td>
                            <td>DB5</td>
                            <td>DB6</td>
                            <td>DB8</td>
                            <td>DB10</td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Pitch (mm)
                            </td>
                            <td>
                               4
                            </td>
                            <td>
                                5
                            </td>
                            <td>
                                6
                            </td>
                            <td>
                                8
                            </td>
                            <td>
                                10
                            </td>
                        </tr>
                        <tr>
                            <td>
                                LED Type
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Size (WxH)/(mm)
                            </td>
                            <td>
                                768x768 or Customized
                            </td>
                            <td>
                                960x960 or Customized
                            </td>
                            <td>
                                768x768 or Customized
                            </td>
                            <td>
                                768x768 or Customized
                            </td>
                            <td>
                                960x960 or Customized
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Per Panel
                            </td>
                            <td>
                                192x192
                            </td>
                            <td>
                                192x192
                            </td>
                            <td>
                                128x128
                            </td>
                            <td>
                                96x96
                            </td>
                            <td>
                                96x96
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Material
                            </td>
                            <td>
                                Magnelium Alloy
                            </td>
                            <td>
                                Magnelium Alloy
                            </td>
                            <td>
                                Magnelium Alloy
                            </td>
                            <td>
                                Magnelium Alloy
                            </td>
                            <td>
                                Magnelium Alloy
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Weight (kg)
                            </td>
                            <td>
                                24
                            </td>
                            <td>
                                34
                            </td>
                            <td>
                                23
                            </td>
                            <td>
                                22
                            </td>
                            <td>
                                34
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Module Size (WxH)/(mm)
                            </td>
                            <td>
                                256x256
                            </td>
                            <td>
                                320x160
                            </td>
                            <td>
                                192x192
                            </td>
                            <td>
                                256x256
                            </td>
                            <td>
                                320x160
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Brightness (nit)
                            </td>
                            <td>
                                6000
                            </td>
                            <td>
                                5000
                            </td>
                            <td>
                                5000
                            </td>
                            <td>
                                5000
                            </td>
                            <td>
                                >5000
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Refresh Rate (Hz)
                            </td>
                            <td>
                                2880
                            </td>
                            <td>
                                2880
                            </td>
                            <td>
                                2880
                            </td>
                            <td>
                                2880
                            </td>
                            <td>
                                2880
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Gray Scale (bit) 
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Viewing Angle (H/V) (°) 
                            </td>
                            <td>
                                160/120
                            </td>
                            <td>
                                160/120
                            </td>
                            <td>
                                160/120
                            </td>
                            <td>
                                160/120
                            </td>
                            <td>
                                160/120
                            </td>
                        </tr>
                        <tr>
                            <td>
                                AC Operating Voltage (V)
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Avg. Power Consumption (W)
                            </td>
                            <td>
                                130/sqm
                            </td>
                            <td>
                                150/sqm
                            </td>
                            <td>
                                150/sqm
                            </td>
                            <td>
                                150/sqm
                            </td>
                            <td>
                                150/sqm
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Max. Power Consumption (W)
                            </td>
                            <td>
                                850/sqm
                            </td>
                            <td>
                                750/sqm
                            </td>
                            <td>
                                750/sqm
                            </td>
                            <td>
                                750/sqm
                            </td>
                            <td>
                                750/sqm
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Storage Temperature (℃) 
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Operating Temperature (℃)
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                        </tr>
                        <tr>
                            <td>
                                IP Rating (Front/Rear)
                            </td>
                            <td>
                                IP65
                            </td>
                            <td>
                                IP65
                            </td>
                            <td>
                                IP65
                            </td>
                            <td>
                                IP65
                            </td>
                            <td>
                                IP65
                            </td>
                        </tr>
                        <tr>
                            <td>
                                LED Lifetime (H)
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Maintenance Method
                            </td>
                            <td>
                                Front or Rear
                            </td>
                            <td>
                                Front or Rear
                            </td>
                            <td>
                                Front or Rear
                            </td>
                            <td>
                                Front or Rear
                            </td>
                            <td>
                                Front or Rear
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Certificate
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
        <div class="related">
            <div class="sub-title">
                <span>Related Products</span>
            </div>
            <div class="product-container">
                <div class="product-item">
                    <a class="thumb" href="../product-detail/ft-series.php">
                        <img src="../images/home/FT SERIES.jpg" class="short"/>
                    </a>
                    <a class="list-title" href="../product-detail/ft-series.php">
                        <span class="current">FT SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/mc-series.php">
                        <img src="../images/home/MC SERIES.jpg"/>
                    </a>
                    <a class="list-title" href="../product-detail/mc-series.php">
                        <span class="current">MC SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/ts-series.php">
                        <img src="../images/home/TS Series.jpg" class="ts-series"/>
                    </a>
                    <a class="list-title" href="../product-detail/ts-series.php">
                        <span class="current">TS SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/ei-series.php">
                        <img src="../images/home/EI Series.jpg"  class="shorter"/>
                    </a>
                    <a class="list-title" href="../product-detail/ei-series.php">
                        <span class="current">EI SERIES</span><span class="more">View More</span>
                    </a>
                </div>
            </div>
        </div>
        <?php include('../inc/foot.php'); ?>
            <!-- 底部结束 -->
    </div>
    <?php include('../inc/js.php'); ?>
    <script>GetCurStyle("products");</script>
</body> 
</html>