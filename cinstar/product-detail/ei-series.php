<!DOCTYPE html>
<html lang="en">
<head>
    <?php include('../inc/meta.php'); ?>
    <?php include('../inc/rem.php'); ?>
    <link rel="stylesheet" href="../css/detail.css">
    <title>LED board manufacturer | cheap LED display, LED video display screen</title>
    <meta name="description" content="Cinstar EI series LED video display screen is specially designed to meet both install and rental applications. Its fast lock system and magnetic modules enable easier installation and maintenance. If you are looking for a LED board manufacturer for a cheap LED display, we may have what you need." />
    <meta name="keywords" content="LED board manufacturer, cheap LED display, LED video display screen, LED video screen price, LED screen for hire" />
</head>
<body class="page-index .menu-open">
    <!-- 头部开始 -->
    <?php include('../inc/top.php'); ?>
    <!-- 头部结束 -->
    <div class="container">
        <div class="detail-title ei-bg bg">
            <div class="title">
                <span>EI SERIES</span>
            </div>
            <div class="top-title">Rental and Install LED Displays</div>
        </div>
        <h1>Cinstar is a professional experienced LED board manufacturer, cheap LED display, LED video display screen, LED video screen price, LED screen for hire, LED screen, LED display, LED video screen, LED video wall, LED video display, LED screen display, LED display screen, LED screen wall, LED display wall, LED wall display, LED wall video, rental LED display, commercial LED display, creative LED display manufacturer.</h1>
        <!-- 产品参数 -->
        <div class="specification">
            <div class="spec-detail">
                <table>
                    <tbody>
                        <tr class="thead">
                            <td colspan="2">Specifications</td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Pitch (mm):
                            </td>
                            <td>
                                3.9 / 4.8 / 5.6 / 5.9
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Brightness (nits):
                            </td>
                            <td>
                                up to 5000
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Refresh Rate(Hz):    
                            </td>
                            <td>
                                >1920
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Application: 
                            </td>
                            <td>
                                Fixed/Rental
                            </td>
                        </tr>
                    </tbody>
                </table>
                <div class="key-feature">
                    <div>Key Features</div>
                    <ul>
                        <li>Designed for both install and rental applications</li>
                        <li>Ultra slim and light weight die-cast aluminum panel</li>
                        <li>Available in both indoor and outdoor options</li>
                        <li>Fast lock system for quick and easy assembly </li>
                        <li>500x500mm and 500x1000mm panels for choice</li>
                        <li>Brilliant visual performance</li>    
                    </ul>
                </div>
            </div>
            <div class="product-img">
                <img src="../images/products/Commercial-LED/EI-SERIES.jpg" alt="EI-SERIES"/>
            </div>
        </div>
        <!-- 产品详情 -->
        <div class="product-desc">
            <div class="left-img">
                <img src="../images/products/Commercial-LED/EI-Case-Show.jpg" alt="EI-Case-Show"/>
            </div>
            <div class="right-desc">
                <h2 class="title">RENTAL AND INSTALL LED DISPLAYS</h2>
                <p>Cinstar EI series is specially designed to meet both install and rental applications. Its classic fast lock system and magnetic modules enable easier installation and maintenance.</p>
                <p>Featuring high refresh rate, high grey scale level and wide color gamut, EI series system brings perfect visual performance to rental productions or commercial advertising.</p>  
                <p>Cinstar EI series system supports any creative installations like hanging, ground stacking, wall mounting or curved to meet any eye-catching visual designs. 
                </p>
            </div>
        </div>
        <!-- SERIES FEATURES -->
        <div class="features">
            <div class="sub-title">
                <span>EI SERIES FEATURES</span>
            </div>
            <div class="iconsList">
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-random"></i>
                    </div>
                    <p class="icon-desc">Designed for both install and rental applications</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="iconfont">&#xe601;</i>
                    </div>
                    <p class="icon-desc">Ultra slim and light weight die-cast aluminum panel</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="iconfont">&#xe68d;</i>
                    </div>
                    <p class="icon-desc">Available in both indoor and outdoor options</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="iconfont">&#xe600;</i>
                    </div>
                    <p class="icon-desc">Fast lock system for quick and easy assembly</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-check"></i>
                    </div>
                    <p class="icon-desc">500x500mm and 500x1000mm panels for choice</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-eye"></i>
                    </div>
                    <p class="icon-desc">Brilliant visual performance</p>
                </div>

            </div>
        </div>
        <!-- showCases -->
        <div class="show-cases">
            <div class="sub-title">
                <span>EI SERIES SHOWCASE</span>
            </div>
            <div class="img-wrap">
                <div class="caseItem">
                    <img src="../images/fixed/2-fixed-case.jpg" alt="US Retail Store EI3.9"/>
                    <a class="case-title">
                        <span class="current">US Retail Store EI3.9 LED Screen Display</span>
                    </a>
                </div>
                <div class="caseItem">
                    <img src="../images/fixed/13-fixed-case.jpg" alt="Italy Shopping Center EI3.9"/>
                    <a class="case-title">
                        <span class="current">Italy Shopping Center EI3.9 LED Screen Display</span>
                    </a>
                </div>
            </div>
        </div>
        <div class="table-detail">
            <div class="sub-title">
                <span>technical specification</span>
            </div>
            <div class="table-wrap">
                <table>
                    <tbody>
                        <tr class="thead">
                            <td></td>
                            <td>EI3.9</td>
                            <td>EI4.8</td>
                            <td>EI3.9(Outdoor)</td>
                            <td>EI4.8(Outdoor)</td>
                            <td>EI5.9(Outdoor)</td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Pitch (mm)
                            </td>
                            <td>
                                3.91
                            </td>
                            <td>
                                4.81
                            </td>
                            <td>
                                3.91
                            </td>
                            <td>
                                4.81
                            </td>
                            <td>
                                5.95
                            </td>
                        </tr>
                        <tr>
                            <td>
                                LED Type
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Size (WxH)/(mm)
                            </td>
                            <td>
                                500x500/500x1000
                            </td>
                            <td>
                                500x500/500x1000
                            </td>
                            <td>
                                500x500/500x1000
                            </td>
                            <td>
                                500x500/500x1000
                            </td>
                            <td>
                                500x500/500x1000
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Per Panel
                            </td>
                            <td>
                                128x128/128x256
                            </td>
                            <td>
                                104x104/104x208
                            </td>
                            <td>
                                128x128/128x256
                            </td>
                            <td>
                                104x104/104x208
                            </td>
                            <td>
                                84x84/84x168
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Material
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Weight (kg)
                            </td>
                            <td>
                                8/13
                            </td>
                            <td>
                                8/13
                            </td>
                            <td>
                                9/14
                            </td>
                            <td>
                                9/14
                            </td>
                            <td>
                                9/14
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Module Size (WxH)/(mm)
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Brightness (nit)
                            </td>
                            <td>
                                1200
                            </td>
                            <td>
                                1200
                            </td>
                            <td>
                                >5000
                            </td>
                            <td>
                                >5000
                            </td>
                            <td>
                                >5000
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Refresh Rate (Hz)
                            </td>
                            <td>
                                >1920
                            </td>
                            <td>
                                >1920
                            </td>
                            <td>
                                >1920
                            </td>
                            <td>
                                >1920
                            </td>
                            <td>
                                >1920
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Gray Scale (bit) 
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Viewing Angle (H/V) (°) 
                            </td>
                            <td>
                                140/140
                            </td>
                            <td>
                                140/140
                            </td>
                            <td>
                                140/120
                            </td>
                            <td>
                                140/120
                            </td>
                            <td>
                                140/120
                            </td>
                        </tr>
                        <tr>
                            <td>
                                AC Operating Voltage (V)
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Avg. Power Consumption (W)
                            </td>
                            <td>
                                230/sqm
                            </td>
                            <td>
                                240/sqm
                            </td>
                            <td>
                                256/sqm
                            </td>
                            <td>
                                248/sqm
                            </td>
                            <td>
                                210/sqm
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Max. Power Consumption (W)
                            </td>
                            <td>
                                690/sqm
                            </td>
                            <td>
                                740/sqm
                            </td>
                            <td>
                                768/sqm
                            </td>
                            <td>
                                744/sqm
                            </td>
                            <td>
                                630/sqm
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Storage Temperature (℃) 
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                            <td>
                                ﹣40~﹢60
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Operating Temperature (℃)
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                        </tr>
                        <tr>
                            <td>
                                IP Rating (Front/Rear)
                            </td>
                            <td>
                                IP40/IP21
                            </td>
                            <td>
                                IP40/IP21
                            </td>
                            <td>
                                IP65/IP54
                            </td>
                            <td>
                                IP65/IP54
                            </td>
                            <td>
                                IP65/IP54
                            </td>
                        </tr>
                        <tr>
                            <td>
                                LED Lifetime (H)
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Module Maintenance 
                            </td>
                            <td>
                                Front
                            </td>
                            <td>
                                Front
                            </td>
                            <td>
                                Front
                            </td>
                            <td>
                                Front
                            </td>
                            <td>
                                Front
                            </td>
                        </tr>
                        <tr>
                            <td>
                                PSU & Others Maintenance
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Certificate
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                            <td>
                                CE FCC ETL RoHS
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
        <div class="related">
            <div class="head-title">
                <span>RELATED PRODUCTS</span>
            </div>
            <div class="product-container">
                <div class="product-item">
                    <a class="thumb" href="../product-detail/lm-series.php">
                        <img src="../images/home/LM SERIES.jpg"/>
                    </a>
                    <a class="list-title" href="../product-detail/lm-series.php">
                        <span class="current">LM SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/hd-series.php">
                        <img src="../images/home/HD SERIES.jpg" class="short"/>
                    </a>
                    <a class="list-title" href="../product-detail/hd-series.php">
                        <span class="current">HD SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/tr-series.php">
                        <img src="../images/home/TR SERIES.jpg" class="short"/>
                    </a>
                    <a class="list-title" href="../product-detail/tr-series.php">
                        <span class="current">TR SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/ft-series.php">
                        <img src="../images/home/FT SERIES.jpg" class="short"/>
                    </a>
                    <a class="list-title" href="../product-detail/ft-series.php">
                        <span class="current">FT SERIES</span><span class="more">View More</span>
                    </a>
                </div>
            </div>
        </div>
        <?php include('../inc/foot.php'); ?>
            <!-- 底部结束 -->
    </div>
    <?php include('../inc/js.php'); ?>
    <script>GetCurStyle("products");</script>
</body> 
</html>